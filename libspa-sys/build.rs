...
fn main() {
    ...
    for micro_version in 0..=100 {
        println!(
            "cargo::rustc-check-cfg=cfg(libpipewire_1_5_{}_or_higher)",
            micro_version
        );
        if pw_check_version(1, 5, micro_version) {
            println!(
                "cargo::rustc-cfg=libpipewire_1_5_{}_or_higher",
                micro_version
            );
        }
    }
